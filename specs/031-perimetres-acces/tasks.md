...
- [x] Branche créée : `feat/perimetres-acces`
- [x] Migration Prisma : **aucune** — cette feature ne touche pas le schéma (voir `plan.md`)

## Tâches

### 1. Modèle de permissions (manifestes de modules)

- [x] **T1** — Déclarer `planning:department` : `SUPER_ADMIN, ADMIN, SECRETARY, MINISTER,
      DEPARTMENT_HEAD` (**sans `STAR`**). Ne **pas** toucher à `planning:view`, qui reste
      accordée au STAR. *(fichier : `src/modules/planning/index.ts`)*
- [x] **T2** — Basculer l'entrée de navigation « Planning » (`/dashboard`) du manifeste sur
      `planning:department`. *(fichier : `src/modules/planning/index.ts`)*
- [x] **T3** [P] — Déclarer `access:manage` : `SUPER_ADMIN, ADMIN, SECRETARY, MINISTER`.
      *(fichier : `src/modules/core/index.ts`)*
- [x] **T4** [P] — Retirer `STAR` de `rooms:view` **et** de `rooms:reserve`.
      *(fichier : `src/modules/rooms/index.ts`)*

### 2. Helpers de périmètre (`src/lib/auth.ts`)

- [x] **T5** — Ajouter `requireDepartmentAccess(session, churchId, departmentId)` : ne retourne
...
- [x] **T6** — Ajouter `getUserMinistryScope(session, churchId)`, symétrique de
...
- [x] **T7** — Remplacer la garde de périmètre recopiée à la main par `requireDepartmentAccess`,
...
- [x] **T8** — `tasks` de département : `GET` passe à `planning:department` ; `POST` et `DELETE`
...
- [x] **T9** — `notices` de département : `GET` passe à `planning:department` ; `PUT` et `DELETE`
      gardent `planning:edit` ; les trois appellent `requireDepartmentAccess`.
      *(fichier : `src/app/api/departments/[departmentId]/notices/route.ts`)*
- [x] **T10** [P] — `members` de département : garde `members:view`, ajouter
      `requireDepartmentAccess`. *(fichier : `src/app/api/departments/[departmentId]/members/route.ts`)*
- [x] **T11** [P] — `stats` de département : passer à `planning:department` + `requireDepartmentAccess`.
      *(fichier : `src/app/api/departments/[departmentId]/stats/route.ts`)*
- [x] **T12** [P] — `monthly-planning` : passer à `planning:department` + `requireDepartmentAccess`.
      *(fichier : `src/app/api/departments/[departmentId]/monthly-planning/route.ts`)*
- [x] **T13** [P] — `tasks` par événement : `GET` passe à `planning:department`, `PUT` garde
...
- [x] **T14** — `planning/weekly` : passer à `planning:department` et **filtrer** la liste sur le
...
- [x] **T15** — Gestion des rôles : remplacer `events:manage` par `access:manage` sur `POST`,
...
- [x] **T16** — Même fichier : appliquer `getUserMinistryScope`. Un appelant au périmètre
...
- [x] **T17** — Dissocier les trois usages aujourd'hui confondus : `hasPlanningAccess` bascule sur
...
- [x] **T18** — Ajouter à `/dashboard` la garde de permission **absente** : exiger
...
- [x] **T19** — Même fichier : `userPermissions` est calculé sur **toutes** les églises de
      ...
      *(fichier : `src/app/(auth)/dashboard/page.tsx`)*
- [x] **T20** — Gestion des accès : garde alignée sur `access:manage` (le Secrétaire peut alors
...
- [x] **T21** — Même page : pour un appelant au périmètre de ministère restreint, ne transmettre
...
- [x] **T22** — Vérifier le parcours **pastoral** : le bloc qui injecte des permissions
...
- [x] **T23** — Helpers : périmètre non restreint (Super Admin, Admin, Secrétaire) ; restreint et
...
- [x] **T24** — Périmètre par département, pour chacun des accès de T8 à T14 : un
...
- [x] **T25** [P] — Périmètre par ministère : un Ministre agit dans son ministère ; est refusé
...
- [x] **T26** [P] — Étanchéité inter-églises de la page de gestion des accès : aucun utilisateur
...
- [x] **T27** [P] — Table des permissions assertée : le STAR n'a **pas** `planning:department`,
...
- [x] **T28** — Réservations orphelines : une réservation créée par un STAR reste visible et
...
- [x] **T29** — Non-régression du parcours STAR conservé : « Mon planning », la vue événements
...
- [x] **T30** — Faire passer les tests de non-régression existants sans autre modification que
...
- [x] **T31** — Rédiger l'**ADR-0009 « Garde de périmètre explicite au point d'entrée »**
      (statut `Accepté`) et l'ajouter à l'index. *(fichiers : `docs/adr/0009-garde-perimetre-explicite.md`,
      `docs/adr/README.md`)*
- [x] **T32** [P] — Mettre à jour le tableau des rôles et permissions : ajouter
...
- [x] **T33** [P] — Ouvrir une **issue de suivi** pour les six pages hors périmètre qui calculent
