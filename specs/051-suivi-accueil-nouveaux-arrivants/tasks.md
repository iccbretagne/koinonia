...
- [ ] Migration Prisma générée (T1, T2)

## Tâches

### 1. Données & migration

- [ ] **T1** — Ajouter au schéma : les deux valeurs d'enum `WAITING_RECONTACT` /
...
- [ ] **T2** — Compter les demandes `SUBMITTED` portant `assignedFamilyId` ou
...
- [ ] **T3** — Créer `family-state.ts` : extraire la machine à états du `switch` de
...
- [ ] **T4** — Dans `family-state.ts`, ajouter `computeReopenData(request, mode, history)` :
      ...
      *(fichier : `src/modules/integration/services/family-state.ts`)*
- [ ] **T5** [P] — Créer `family-history.ts` : `recordStatusChange({ userId, churchId, requestId,
...
- [ ] **T6** [P] — Dans `family-history.ts`, ajouter `getRequestHistory(requestId)` : lit
      ...
      *(fichier : `src/modules/integration/services/family-history.ts`)*
- [ ] **T7** — Étendre `family-service.ts` : `getIntegrationSettings(churchId)` (lecture avec
...
- [ ] **T8** — Étendre `family-service.ts` : `runWaitingRelanceNotifications(appUrl)`, en miroir
      ...
      *(fichier : `src/modules/integration/services/family-service.ts`)*
- [ ] **T9** [P] — Ajouter `buildRelanceEmail(...)` sur le modèle de `buildInactivityEmail`
      (mêmes conventions : `churchName`, cible de la relance, lien, `appUrl`).
      *(fichier : `src/modules/integration/services/family-service.ts`)*
- [ ] **T10** [P] — Ajouter `requireIntegrationSettingsAccess(churchId)` dans `auth.ts` : Super
...
- [ ] **T11** — Mettre à jour `src/modules/integration/index.ts` pour exporter les nouvelles
...
- [ ] **T12** — Réécrire le `switch` de `PATCH /api/integration/requests/[id]` pour appeler
...
- [ ] **T13** [P] — Créer `GET /api/integration/requests/[id]/history`, gardé par
      `requireIntegrationAccess`, appelant `getRequestHistory` (T6).
      *(fichier : `src/app/api/integration/requests/[id]/history/route.ts`)*
- [ ] **T14** [P] — Créer `GET`/`PUT /api/integration/settings`, gardé par
...
- [ ] **T15** — Ajouter `contactConsent: z.enum(["NOW", "LATER"]).default("NOW")` au
...
- [ ] **T16** — Brancher `runWaitingRelanceNotifications` dans le cron, à côté de
      `runInactivityNotifications`.
      *(fichier : `src/app/api/cron/route.ts`)*

### 4. UI

- [ ] **T17** [P] — Formulaire public : ajouter le choix « Être contacté maintenant / Être
...
- [ ] **T18** [P] — Dashboard : ajouter les filtres de statut `WAITING_RECONTACT` /
...
- [ ] **T19** — Fiche demande : boutons contextuels `Mettre en attente` (choix
...
- [ ] **T20** — Frise d'historique : nouveau composant affichant les entrées de
...
- [ ] **T21** [P] — Page réglages : deux `Input type="number"` (délai recontact, délai mission) +
...
- [ ] **T22** [P] — Ajouter le lien « Paramètres » vers `/integration/parametres` dans la
...
- [ ] **T23** — Tests de `computeFamilyTransitionData` : table complète des arcs existants
...
- [ ] **T24** [P] — Tests de `computeReopenData` : `mode: "resume"` restaure le statut
      ...
      *(fichier : `src/modules/integration/services/__tests__/family-state.test.ts`)*
- [ ] **T25** [P] — Test de `assertNoStaleAssignment` : rejette une donnée `SUBMITTED` portant
      une famille ou un berger.
      *(fichier : `src/modules/integration/services/__tests__/family-state.test.ts`)*
- [ ] **T26** [P] — Tests de `getRequestHistory` : reconstitue plusieurs cycles attente → reprise
...
- [ ] **T27** — Tests de la sélection des relances (fonction de calcul d'échéance extraite de
...
- [ ] **T28** [P] — Tests de `requireIntegrationSettingsAccess` : Admin et Secrétaire passent,
...
- [ ] **T29** [P] — Test du schéma Zod de `POST /api/integration/requests` : `contactConsent`
...
- [ ] **T30** — Test de migration T2 : sur un fixture reproduisant l'état bâtard (`SUBMITTED` +
...
- [ ] `npm run test`
- [ ] `npm run db:migrate:deploy` rejoué sur base vierge (CI `migrations`) sans erreur
