...
## Phase 3 — Parcours self-service refondu + matching flou de la recherche

- **Statut** : **Livrée.** Le parcours self-service était déjà couvert par P1 + P2 + le flux
  existant (voir tableau ci-dessous) ; P3 apporte en plus le **matching flou (tokens + tolérance
  aux fautes)** de la recherche de fiches STAR, qui corrigeait un défaut du matching initial
  (comparaison de la requête entière à `firstName contains … OR lastName contains …` → 0 résultat
  pour « Prénom Nom » alors qu'une fiche existait → risque de doublon).

**Tâches faites (matching flou)** :

- Helpers purs et testables dans `src/lib/onboarding.ts` :
  - `levenshtein(a, b)` — distance d'édition itérative O(n·m), sans dépendance externe.
  - `tokenMatchScore(query, firstName, lastName)` — score 0..1, tolérant à l'ordre des mots, aux
    accents (via `normalizeName`) et aux fautes ; seuil par-token `TOKEN_MATCH_THRESHOLD = 0.7`.
  - `rankMembersByName(query, members, opts)` — classe par score décroissant puis nom, filtre sous
    le seuil (0.7), limite (défaut 10) ; score exposé via `_score`.
- Réécriture de `GET /api/members/search` : récupère les fiches **non liées** de l'église
  (`take: 500`), applique `rankMembersByName` côté application, renvoie le top 10. Garde
  `requireChurchAccess`, la validation `churchId`/`q.length < 2`, l'exclusion des fiches déjà liées
  et le contrat de réponse inchangé.
- Tests (`src/lib/__tests__/onboarding.test.ts`) : `levenshtein`, `tokenMatchScore` (match exact,
  ordre inversé, token unique, faute « Dupnt », accents, faux positif « Xavier Bernard » → 0) et
  `rankMembersByName` (tri, exclusion sous seuil, limite).
...
**Conclusion** : la refonte est fonctionnellement complète avec P1 + P2 + le matching flou de P3.
Reste la **validation manuelle en recette** puis la **PR finale `feat/onboarding-liaison-email` →
`main`**.
