...
- [x] Branche `feat/modules-optionnels-deploiement` (existante — spec et plan y sont déjà)
- [x] **Aucune migration Prisma** : la feature ne touche pas le schéma (`plan.md` § Modèle de données)

## Tâches

### 1. Déclarer — la surface HTTP devient une donnée du manifeste

- [x] **T1** — Créer l'algorithme de résolution de route : table de préfixes construite depuis une
...
- [x] **T2** [P] — Extraire le tableau des 11 manifestes dans un fichier dédié, importés depuis
...
- [x] **T3** — Remplir le champ `routes` (`authenticated` / `public` / `api`) des **11 manifestes**
...
- [x] **T4** — Créer la racine de composition légère : `NOYAU_ROUTES` (`/`, `/no-access`,
...
- [x] **T5** — **Test d'exhaustivité** *(CA16)*. Parcourt le système de fichiers
...
- [x] **T6** [P] — Test de résolution : préfixe le plus long (`/api/admin/media` → `media` et non
...
- [x] **T7** [P] — Page 404 globale de l'application, aux couleurs ICC, sans composant nouveau
...
- [x] **T8** [P] — Relais de rendu : page appelant `notFound()`, cible de la réécriture interne du
...
- [x] **T9** — Contrôle de module dans le proxy, **en tout premier**, avant la lecture du cookie de
...
- [x] **T10** — Remplacer la liste blanche des routes publiques codée en dur dans le proxy par la
...
- [x] **T11** — Élargir `config.matcher` à toute l'application, en excluant `_next/static`,
...
- [x] **T12** — Ajouter une règle `dependency-cruiser` `proxy-only-module-routes` interdisant à
...
- [x] **T13** — Trois validations *fail-fast* dans `boot()` *(CA6, CA7, CA8, CA9)* : le module
...
- [x] **T14** — Séparer les deux abonnements du bus selon la règle **nettoyer oui, créer non**
...
- [x] **T15** — Conditionner les travaux planifiés *(CA11)* : les imports statiques de
...
- [x] **T16** — Corriger la redirection d'atterrissage du layout authentifié, aujourd'hui codée en
...
- [x] **T17** — Élargir la garde `/no-access` (ligne 47) *(CA10)* : elle ne se déclenche
...
- [x] **T18** — Tests de démarrage *(CA6, CA7, CA8, CA9)* : sans liste → 11 modules actifs ; liste
...
- [x] **T19** — Étendre les tests du proxy *(CA1, CA2, CA3, CA4, CA5)* : module inactif → page
...
- [x] **T20** [P] — Tests des abonnements du bus *(CA12, CA13)* : `planning:event:cancelled`
...
- [x] **T21** [P] — Tests du cron *(CA11)* : sans `integration` ni `jobs`, la route répond 200 et
...
- [x] **T22** [P] — Tests du layout authentifié *(CA10)* : utilisateur dont les rôles ne portent que
...
- [x] **T23** [P] — Tests de non-régression de la matrice et de la navigation *(CA14, CA17)* :
...
- [x] **T24** — **ADR-0012** — « Le manifeste déclare la surface HTTP du module ; le proxy
...
- [x] **T25** [P] — Documentation d'exploitation (question ouverte 2 de la spec, tranchée **dans**
...
- [x] **T26** [P] — Mettre à jour `CLAUDE.md` : le champ `routes` du manifeste est désormais
...
- [x] **T27** — Vérification manuelle sur une instance réduite : démarrer avec
...
- [x] `npm run typecheck`
- [x] `npm run lint`
- [x] `npm run lint:boundaries` — dont la nouvelle règle `proxy-only-module-routes` (T12)
- [x] `npm run test`
- [x] `npm run build` — le proxy est chargé au build, une erreur de composition s'y verrait
- [x] T27 exécutée (CA15, vérification manuelle)
- [x] Tous les critères d'acceptation de `spec.md` satisfaits (voir table ci-dessus)
- [x] Case ADR de la checklist de `plan.md` cochée (T24 faite)
