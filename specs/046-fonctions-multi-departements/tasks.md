...
- [x] **T1** — Dans `src/lib/department-functions.ts`, ajouter `MSDP` à `DEPT_FN` s'il manque,
...
- [x] **T2** — Créer `src/lib/function-departments.ts` (serveur) : `getFunctionDepartmentIds(churchId,
...
- [x] **T3** — Ajouter (à côté de T1) `formatAssignedDepts(fn, depts)` (pur) → « Nom » / « A, B » /
...
- [x] **T4** [P] — Audio : remplacer `getCaptureDepartmentId` par `getCaptureDepartmentIds`
...
- [x] **T5** [P] — Intégration : `getManagers` (relance demandes sans suite) récupère les
...
- [x] **T6** [P] — MSDP : même traitement pour `getMsdpManagers`.
      *(fichier : `src/modules/integration/services/msdp-service.ts`)*

### 3. API (route handlers)

- [x] **T7** [P] — `PATCH /api/departments/[departmentId]` : supprimer le `updateMany` qui
...
- [x] **T8** — `POST /api/requests` (VISUEL et demandes Secrétariat) : retirer les `findFirst`
...
- [x] **T9** — `GET/PATCH /api/requests/[id]` : `isAssignedDeptMember` devient
...
- [x] **T10** — `POST /api/announcements` : `findDeptByFunction` remplacé par un contrôle
...
- [x] **T11** [P] — File Secrétariat : garde d'accès via `getFunctionDepartmentIds` (vide →
...
- [x] **T12** [P] — File Demandes visuels : idem, file `type: VISUEL`.
      *(fichier : `src/app/(auth)/media/requests/page.tsx`)*
- [x] **T13** [P] — File Demandes réseaux sociaux : idem, file `type: RESEAUX_SOCIAUX`.
      *(fichier : `src/app/(auth)/communication/requests/page.tsx`)*
- [x] **T14** — Mes demandes : `requests/page.tsx` charge `assignedDepts`/`assignedFunction`
...
- [x] **T15** — Configuration des fonctions : `<select>` remplacé par `CheckboxGroup` groupé par
...
- [x] **T16** [P] — Helpers : `REQUEST_TYPE_FUNCTION` couvre toutes les valeurs de l'enum
...
- [x] **T17** [P] — `function-departments` : 0 / 1 / N départements, isolation par église (le
...
- [x] **T18** [P] — PATCH département : assigner une fonction n'appelle aucune mise à jour sur un
      autre département. *(fichier : `src/app/api/departments/__tests__/route.test.ts`)*
- [x] **T19** [P] — `/api/requests/[id]` : membre d'un 2ᵉ département de la fonction → autorisé ;
...
- [x] **T20** [P] — `POST /api/announcements` : fonction portée par 2 départements → création
      sans `assignedDeptId` ; aucune → 400 inchangé.
      *(fichier : `src/app/api/announcements/__tests__/security.test.ts`)*
- [x] **T21** [P] — Audio : membre et responsable d'un 2ᵉ département de captation → accès et
      dépublication ; hors départements → refus.
      *(fichier : `src/modules/audio/services/__tests__/access.test.ts`)*
- [x] **T22** [P] — Relances intégration et MSDP : deux départements, une personne dans les deux
...
- [x] **T23** — Mettre à jour les tests existants cassés par la disparition d'`assignedDeptId` /
      `getCaptureDepartmentId` (sans affaiblir leurs assertions de sécurité).

### 6. Documentation

- [x] **T24** [P] — `CLAUDE.md` et `docs/auth.md` : « le département de captation » et les
