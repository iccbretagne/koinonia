...
- [x] **T1** — `NotificationEmailPreference` (clé composite `userId`+`domain`, `enabled`,
      `updatedAt`) et champ `Notification.domain` (nullable) + index `[userId, domain]`.
      *(fichier : `prisma/schema.prisma`)*
- [x] **T2** — Migration `add_notification_email_preferences` (`prisma migrate dev
...
- [x] **T3** — Rejouer T1–T2 sur MariaDB locale avec les fixtures de dev ; vérifier par sondage
...
- [x] **T4** — `NotificationDomainDescriptor` (`key`, `label`, `description`, `defaultEmail`,
      `visibleWith?: readonly Permission[]`) et `ModuleManifest.notificationDomains?`.
      *(fichier : `src/core/module-registry.ts`)*
- [x] **T5** — `ModuleRegistry.collectNotificationDomains()` : agrège les domaines des modules
      actifs, `throw` si deux modules déclarent la même clé — miroir de `collectPermissions()`.
      *(fichier : `src/core/module-registry.ts`)*
- [x] **T6** [P] — `src/core/notification-domains.ts` : `buildNotificationDomains(registry)` trie
      le résultat par point de code (miroir `buildRolePermissions`, `src/core/permissions.ts`).
      *(fichier nouveau)*
- [x] **T7** — `notificationDomains = buildNotificationDomains(registry)` exposé à côté de
      `rolePermissions`. *(fichier : `src/lib/registry.ts`)*
- [x] **T8** [P] — Domaines `planning` (« Planning et service », `defaultEmail: true`,
...
- [x] **T9** [P] — Domaine `care` (« Suivi pastoral », `defaultEmail: true`, `visibleWith:
      ["care:qualify", "care:view"]`). *(fichier : `src/modules/care/manifest.ts`)*
- [x] **T10** [P] — Domaine `integration` (« Intégration », `defaultEmail: true`, `visibleWith:
      ["members:manage", "events:manage"]`). *(fichier : `src/modules/integration/manifest.ts`)*
- [x] **T11** [P] — Domaine `accounting` (« Comptabilité », `defaultEmail: true`, `visibleWith:
      ["accounting:submit", "accounting:view", "accounting:manage"]`).
      *(fichier : `src/modules/accounting/manifest.ts`)*
- [x] **T12** [P] — Domaine `rooms` (« Salles », `defaultEmail: false`, `visibleWith:
      ["rooms:manage"]`). *(fichier : `src/modules/rooms/manifest.ts`)*
- [x] **T13** [P] — Domaine `media` (« Médias », `defaultEmail: false`, `visibleWith:
...
- [x] **T14** [P] — Domaine `jobs` (« Emploi », `defaultEmail: true`, `visibleWith:
...
- [x] **T15** [P] — Domaine `account` (« Compte et accès », `defaultEmail: false`,
      `visibleWith` omis = toujours affiché). *(fichier : `src/modules/core/manifest.ts`)*

### 1.3 Services communs

- [x] **T16** — `src/lib/notification-preferences.ts` : `resolveEmailPreference({ globalEnabled,
...
- [x] **T21** — `src/lib/notifications.ts` : `createNotification`, `notifyUsersWithRole`,
...
- [x] **T22** — Comptabilité, paiement remis à `payment.request.submittedById` :
      `dispatchUserEmails`/`notifyUsers(..., { domain: "accounting" })`.
      *(fichier : `src/app/api/accounting/payments/[id]/route.ts`)*
- [x] **T23** — Comptabilité, changement de statut à `req.submittedById` : même migration.
      *(fichier : `src/app/api/accounting/requests/[id]/route.ts`)*
- [x] **T24** — Comptabilité, nouvelle demande : la notification in-app aux comptables
...
- [x] **T25** — Rappels de service (orchestrateur actif) : `runReminders()` envoie aujourd'hui
...
- [x] **T26** — Rappels de service (endpoint remplacé par l'orchestrateur ci-dessus selon le
...
- [x] **T27** [P] — Suivi pastoral : les six fonctions (`notifyAssigneeAssigned`,
...
- [x] **T28** [P] — Suivi pastoral, relances et inactivité MSDP : `domain: "care"`.
      *(fichiers : `src/modules/care/services/relances.ts`, `src/modules/care/services/followups.ts`)*
- [x] **T29** [P] — Intégration : berger affecté/dessaisi, renvoi à l'équipe, relances et
      inactivité — `domain: "integration"`. *(fichier : `src/modules/integration/services/family-service.ts`)*
- [x] **T30** [P] — Emploi, nouvelle offre : la notification in-app et l'email passent par
...
- [x] **T31** [P] — Emploi, relance de renouvellement d'offre : l'email à `offer.author.email` est
