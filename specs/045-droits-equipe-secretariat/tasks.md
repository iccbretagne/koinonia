...
- [x] Migration Prisma : **aucune** — étape 1 ne touche pas au schéma (voir `plan.md`)

## Tâches

### 1. Session — rôle synthétique (le cœur du changement)

- [x] **T1** — Dans `src/lib/auth.ts`, étendre la déclaration `Session.user.churchRoles` (ligne
      ~53-62) avec un champ `virtual?: boolean` optionnel, pour marquer une entrée non persistée
      en base. *(fichier : `src/lib/auth.ts`)*
- [x] **T2** — Dans le callback `session` (`src/lib/auth.ts:130-244`), après le calcul de
...
- [x] **T3** — Toujours dans le callback `session`, après la construction de
...
- [x] **T4** — Vérifier que `GLOBAL_ROLES` (`src/lib/auth.ts:261`) traite déjà `SECRETARY` comme
...
- [x] **T5** — Dans `src/app/api/users/[userId]/roles/route.ts`, vérifier que `POST`/`PATCH`/
...
- [x] **T6** — Confirmer (par lecture, sans modification si déjà correct) que l'anti-escalade
...
- [x] **T7** [P] — Dans `src/app/(auth)/admin/access/AccessClient.tsx`, filtrer/annoter les
...
- [x] **T8** [P] — Même traitement dans `src/app/(auth)/admin/users/UsersClient.tsx` : une entrée
...
- [x] **T9** [P] — Mettre à jour `src/components/GuideContent.tsx` et `src/lib/tour-steps.ts`
...
- [x] **T10** [P] — Mettre à jour les commentaires de
...
- [x] **T11** — Test de résolution de session (nouveau fichier, ex.
...
- [x] **T12** — Test paramétré de parité : pour chacune des 28 permissions du rôle `SECRETARY`
...
- [x] **T13** — Test des fonctions de périmètre : `getUserDepartmentScope`,
      ...
      `src/lib/__tests__/auth-secretariat-role.test.ts`)*
- [x] **T14** — Tests de non-régression : un STAR d'un autre département, un Responsable de
...
- [x] **T15** — Test anti-escalade : un membre de l'équipe (session avec entrée synthétique
...
- [x] **T16** — Confirmer sans modification que les tests existants de
...
- [x] **T17** — Confirmer sans modification que `src/core/__tests__/permissions.test.ts` (matrice
...
- [x] `npm run test`
- [x] `npm run build` (le changement touche `src/lib/auth.ts`, partagé serveur/pages — voir
      `feedback_build_verifie_frontiere_client_serveur.md`)
- [x] Tous les critères d'acceptation de `spec.md` satisfaits (les 12 items de la checklist)
- [x] Statut de la spec passé à `Implémentée`
- [x] PR ouverte vers `main`, référençant `specs/045-droits-equipe-secretariat/`
