...
- [x] Branche créée : `feat/absence-ciblee`
- [x] Migration Prisma générée (schéma modifié)

## Tâches

### 1. Données & migration

- [x] **T1** — Modifier le schéma :
...
- [x] **T2** — Générer la migration `add_absence_targeting` avec `npm run db:migrate`.
...
- [x] **T3** — Créer le module de ciblage :
...
- [x] **T4** — Ajouter `validateTargeting(tx, { churchId, memberId, kind, eventIds, allDepartments, departmentIds, declarerScope })`.
  Contrôles :
  ...
  *(fichier : `src/modules/planning/services/absence-targeting.ts`)*
- [x] **T5** — Ajouter `listTargetOptions(churchId, memberId, declarerScope, { from, to })`.
  ...
  *(fichier : `src/modules/planning/services/absence-targeting.ts`)*
- [x] **T6** — Adapter `findAbsenceConflicts` pour qu'il reçoive un objet de ciblage
...
- [x] **T7** — `resolveResponsibleUserIds` : ajouter un paramètre optionnel `departmentIds`,
...
- [x] **T9** — `updateAbsence` :
  ...
  *(fichier : `src/modules/planning/services/absence.service.ts`)*
- [x] **T10** — `cancelAbsence` : notifier les responsables des départements effectifs.
...
- [x] **T12** — Déplacer `findActiveAbsencesByMember` de la route planning vers le module, sous
  ...
  *(fichier : `src/modules/planning/services/absence-targeting.ts`)*
- [x] **T13** — `findActiveAbsenceForMember` (ouverture/fermeture) : ne prendre en compte que les
...
- [x] **T14** — Ajouter le filtre de visibilité et de filtrage :
  ...
  *(fichier : `src/modules/planning/services/absence-targeting.ts`)*
- [x] **T15** — Exporter les nouvelles fonctions et types depuis l'index du module.

  *(fichier : `src/modules/planning/index.ts`)*

### 3. API (route handlers)

- [x] **T16** — POST `/api/absences` :
...
- [x] **T17** — GET `/api/absences` :
  ...
  *(fichier : `src/app/api/absences/route.ts`)*
- [x] **T18** — PATCH `/api/absences/[id]` : ajouter les champs de ciblage, optionnels, à la
  branche `update`.
...
- [x] **T20** [P] — Nouvelle route `GET /api/absences/target-options` : gardes identiques à
...
- [x] **T21** [P] — Route planning : utiliser `findActiveAbsencesForPlanning` depuis
...
- [x] **T22** — Formulaire de déclaration et de modification :
...
- [x] **T23** — Tableau :
  ...
  *(fichier : `src/app/(auth)/absences/AbsencesClient.tsx`)*
- [x] **T24** [P] — Frise : barre pour une période, marqueurs aux dates des événements existants
  pour des événements ; événements supprimés ignorés.

  *(fichier : `src/app/(auth)/absences/AbsencesTimeline.tsx`)*
- [x] **T25** [P] — `AbsenceBadge` : libellé « Absent · période » ou « Absent · cet événement »
  selon `kind`.

  *(fichier : `src/components/PlanningGrid.tsx`)*
- [x] **T26** — Vérification mobile à 375 px :
...
- [x] **T27** [P] — Tests du module de ciblage :
...
- [x] **T28** — Tests du service d'absence :
...
- [x] **T30** [P] — Route planning : `activeAbsence` présent pour le département et l'événement
...
- [x] **T31** [P] — Ouverture/fermeture : `absenceWarning` levé pour une absence « tous
...
- [x] **T32** [P] — Documenter le nouveau schéma (tables `absence_departments` et
  `absence_events`), la route `target-options` et les champs de ciblage.

  *(fichiers : `docs/database.md`, `docs/api.md`)*
- [x] **T33** [P] — Mettre à jour le CHANGELOG, section « Non publié » : absence ciblée, et
...
- [x] `npm run typecheck` (aucun `!` pour contourner la nullabilité des dates)
- [x] `npm run lint`
