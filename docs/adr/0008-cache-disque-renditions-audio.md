...
- La route de streaming sert le fichier elle-même, en honorant le `Range` HTTP — le seek et la
  reprise d'écoute (spec 021) en dépendent.
...
applicative pendant toute sa durée, là où la redirection la libérait immédiatement. Le
streaming doit être servi en flux, sans charger le fichier en mémoire, sous peine de faire
tomber le serveur sur quelques écoutes simultanées.
