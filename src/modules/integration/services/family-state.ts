...
const WAITING_ENTRY_STATUSES: FamilyIntegrationStatus[] = ["SUBMITTED", "CONTACTED"];
 ...
 * (aucun berger désigné), berger assigné ou équipe depuis « premier contact établi ».
 * Calculé sur le point d'entrée, pas sur l'état courant.
 */
function requireWaitingRight(origin: FamilyIntegrationStatus | null, actor: FamilyActor) {
  if (origin === "CONTACTED") requireBergerOrIntegrationMember(actor);
  ...
  switch (body.action) {
    ...
    case "wait":
      if (!WAITING_ENTRY_STATUSES.includes(current.status))
        throw new ApiError(
          400,
          "Transition invalide : seule une demande reçue ou au premier contact établi peut être mise en attente"
        );
    ...
    case "resume":
      ...
      // famille ; depuis CONTACTED, l'ajout au groupe — dans les deux cas, l'état d'origine.
      result.data = {
